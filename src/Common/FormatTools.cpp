...
std::string get_status_string(CryptoNote::COMMAND_RPC_GET_INFO::response iresp) {
  ...
  //auto forkStatus = get_fork_status(iresp.height, iresp.upgrade_heights, iresp.supported_height);

  ss << "Height: " << iresp.height //<< "/" << iresp.height
     << " (" << get_sync_percentage(iresp.height, iresp.height - 1) << "%), "
     //<< "on " << (iresp.testnet ? "testnet, " : "mainnet, ")
     //<< (iresp.synced ? "synced, " : "syncing, ")
     << "net hash " << get_mining_speed(iresp.difficulty / CryptoNote::parameters::DIFFICULTY_TARGET) << ", "
     << "v" << +iresp.block_major_version << ","
     //<< get_update_status(forkStatus, iresp.height, iresp.upgrade_heights)
