...
bool Blockchain::prevalidate_miner_transaction(const Block& b, uint32_t height) {
  ...
  uint32_t currentMoneyUnlockWindow = height >= CryptoNote::parameters::UPGRADE_HEIGHT_V5 ? m_currency.minedMoneyUnlockWindow_v1() : m_currency.minedMoneyUnlockWindow();
  if (!(b.baseTransaction.unlockTime == height + currentMoneyUnlockWindow)) {
    logger(ERROR, BRIGHT_RED)
      << "coinbase transaction transaction have wrong unlock time="
      << b.baseTransaction.unlockTime << ", expected "
      << height + currentMoneyUnlockWindow;
    return false;
  }
...
size_t Blockchain::find_end_of_allowed_index(const std::vector<std::pair<TransactionIndex, uint16_t>>& amount_outs) {
  ...
  do {
    --i;
    if (amount_outs[i].first.block + (getCurrentBlockchainHeight() >= CryptoNote::parameters::UPGRADE_HEIGHT_V5 ? m_currency.minedMoneyUnlockWindow_v1() : m_currency.minedMoneyUnlockWindow()) <= getCurrentBlockchainHeight()) {
      return i + 1;
    }
  } while (i != 0);
